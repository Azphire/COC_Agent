...
export default function ModulePreparationPage() {
  ...
  return <>
    <section><h2>模组准备工作台</h2><p>选择已索引模组或指定范围，按章节串行生成草稿，逐项核对证据与公开摘要。批准后可在未开始的房间绑定。</p><p>每段保持上下文预算；重试复用已完成段落。生成完成仍需按全文覆盖清单核对。</p></section>
...
function EntityEditor({ entity, token, busy, command, initial, onInitial, required, onRequired }: EditorProps) {
  ...
  const [combat, setCombat] = useState(JSON.stringify(entity.combat_template || null, null, 2))
  ...
  return <article className="entity-card" data-entity-id={entity.id}>
    ...
    {entity.reviewed_by && <p>校对者：{entity.reviewed_by} · 依据：{entity.review_basis} · 来源块 {entity.source_block_ids?.length || 0} 个</p>}
    <form onSubmit={e => {
      e.preventDefault(); setError('')
      try { void command(`/module-entities/${entity.id}`, { title, type, keeper_summary: keeper, public_summary: summary, initial_visibility: visibility, combat_template: type === 'npc' ? JSON.parse(combat) : null, check_stats: type === 'npc' ? JSON.parse(stats) : null, sanity_effects: JSON.parse(sanity), suggested_checks: JSON.parse(checks), reveal_conditions: { ...JSON.parse(conditions), access_policy: accessPolicy } }, 'PATCH') }
      ...
      {type === 'npc' && <details><summary>NPC 战斗模板与缺项</summary><p>模板随批准版本冻结；房间中的 HP、伤势和弹药独立保存。缺失数值必须保留，不会自动补成普通敌人。</p><label>战斗模板（JSON，null 表示未准备）<textarea value={combat} disabled={!editable} onChange={e => setCombat(e.target.value)} /></label></details>}
      {entity.interactions && entity.interactions.length > 0 && <details><summary>已配置的物品／事件交互（{entity.interactions.length}）</summary><pre>{JSON.stringify(entity.interactions, null, 2)}</pre></details>}
      ...
      <details><summary>编辑检定与公开条件</summary><p>实体引用填写本任务的实体 ID；留空表示没有额外限制。检定名称使用角色卡技能键。</p><label>SAN 效果（JSON，保存后重新批准）<textarea value={sanity} disabled={!editable} onChange={e => setSanity(e.target.value)} /></label><p>每项填写 id、encounter、trigger（action_target / entity_revealed）、success_loss、failure_loss、source、page、basis、visibility。同时填写 automation（automatic / host_review）、repeat（first_only / host_confirmed）、action_types 与 condition。默认须主机确认、仅首次触发。已批准效果可设置 kp_enabled=true，由 KP 引用实际行动与已有事实裁定文字条件；这不批准新增数值。audience 可为 actor 或 party。其余条件默认等待主机确认，新行动不会自动视为重复遭遇。常数与骰式只接受非负结果；请按来源明确配置。 四种经历免疫仅在来源明确时填写 experience_category（witness_corpse、witness_severe_injury、witness_murder、commit_murder、witness_human_mutilation），并填写 experience_basis 说明损失仅由该独立原因引起；目击类须 perception=visual。混合恐怖、尸体复活或超出范围者留空，不能因 mythos=false 设为免疫。旧包不自动分类。</p><label>建议检定（JSON）<textarea value={checks} disabled={!editable} onChange={e => setChecks(e.target.value)} /></label><label>公开条件（JSON）<textarea value={conditions} disabled={!editable} onChange={e => setConditions(e.target.value)} /></label></details>
