...
import MythosExperience from './MythosExperience'
...
export default function ExperiencePackages({ character, ruleset, value, onChange }: {
  ...
  return <section data-testid="experience-packages"><h2>可选经历包</h2>
    ...
    <label>经历包<select id="experience-package" value={selection?.package ?? ''} onChange={e => {
      ...
      onChange({ ...value, experience: next ? { package: next.key, variant: Object.keys(next.variants)[0] ?? '', history: '', background_kind: 'scar', background_detail: '', choices: {}, war_year: null, scenario_year: null, age_at_war: null, mythos: next.key === 'mythos' ? { knowledge: 'reading', belief: 'believer', method: 'manual', value: 0, backgrounds: [{ kind: 'scar', detail: '' }, { kind: 'encounter', detail: '' }], spells: [] } : null } : null, approve_experience: undefined })
    ...
    {policy && selection && <>
      ...
      {selection.package !== 'mythos' && <><p>额外 {policy.points} 点，初始 SAN 减少 {policy.san_loss}；首次保存所选包时掷骰，撤销再选及改资料复用原骰。</p>
      ...
      {selection.package === 'mythos' && selection.mythos && <MythosExperience value={selection.mythos} onChange={mythos => change({ mythos })} />}
