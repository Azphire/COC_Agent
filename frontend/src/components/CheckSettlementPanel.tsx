...
export default function CheckSettlementPanel({ check, room, busy, command }: Props) {
  ...
  return <div className="check-settlement">
    ...
    {p.review && <p>主机{p.review.approve ? '核准' : '拒绝'}：{p.review.reason}。{p.review.consequence && `若失败：${p.review.consequence.description}`}</p>}
    ...
    {check.status === 'pending' && p.stage === 'choice' && canChoose && <>
      ...
      {!!check.options?.luck.length && <form onSubmit={e => { e.preventDefault(); void command(path + '/choice', { operation: 'luck', spend: Number(spend) }) }}>
        <label>幸运消耗（可选规则）<select required value={spend} onChange={e => setSpend(e.target.value)}><option value="">选择花费与结果</option>{check.options.luck.map(o => <option key={o.spend} value={o.spend}>花费 {o.spend} → {o.result.total} · {resultLabels[o.result.level]} · {o.result.passed ? '通过' : '未通过'}</option>)}</select></label><button disabled={disabled || !spend}>确认扣除 Luck 并结算</button>
      </form>}
    ...
    {p.stage === 'push_review' && <p>等待主机核准资格及更严重的失败后果。</p>}
    {p.stage === 'push_review' && room.is_host && <form onSubmit={e => { e.preventDefault(); void command(path + '/push-review', { approve: true, reason, consequence: { kind, description, condition, minutes: kind === 'time' ? Number(minutes) : null } }) }}>
