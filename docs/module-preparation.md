...
只处理已有索引中的文本，不读取图片、地图、OCR，不下载模型或调用外部模型。大模组须缩小范围；本地模型仍可能遗漏实体、误分类或生成不恰当的公开摘要，必须人工核对。没有完整模组编辑器、关系图、战斗或自动审批。工作台保留最近 30 条准备完成、实体编辑／批准／拒绝及版本批准记录；它不是完整内容管理审计系统。
