...
第六批《常暗之厢》只校正开场和一次邻接转换，旧记录在 `.cache/batch-6/常暗之厢-session.md`。第十六批已核对全文，批准7场景、12相邻转换及终态配置；完整原稿/配置仍不提交Git，长期包在 `data/prepared/changan/batch-16/`。最新来源覆盖、加载方法和实际运行限制见 [第十六批报告](batch-16-report.md)。

第十六批转换可配置持有物和事件标志条件；权威导航同步玩家和NPC参战位置，保留HP、伤势、弹药及遭遇回执。未完成攻击/伤势和激活战斗阻止普通转场；先完成结算/脱离再移动。被原文事件实际毁去的节点阻断回访。结局和待结算终幕为独立持久状态，不能伪装成回到开场的导航操作。明确目的地与批准出口交叉核对，防止模型选择反方向。

- 结构不等于全部剧情语义；漏检标题、低置信度和单节点 fallback 仍需人工审阅。本批没有任意正文切分/新增节点编辑器。
- Word 内部链接未全部解析为转换，复杂表格合并和 PDF 表格检测不保证完整；无图片、OCR、地图识别。
- 保留既有单 worker 广播和串行模型；战斗已由第十五/十六批接通，不扩展追逐、多模型。
