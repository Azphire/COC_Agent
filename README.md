...
首页提供“继续游戏／开始新游戏／加入朋友”，默认单机。已有批准模组和配置时，正常开团在一个向导内完成：

1. **选模组**：选择已批准准备版本。首次主机解锁、缺失模型配置及规则来源选择都在向导内完成；可直接读取本机已有规则文件。已有有效设置复用，不自动下载或切换模型。
2. **配队伍**：选择自己的已确认角色和 AI 人数。没有角色可在此快速生成或手动建卡。随机队友无需人物概念，包含完整合法数值卡、职业技能、信用、装备及背景、性格、动机、说话风格和行动倾向。可重抽此人、全部重抽或编辑，单人重抽不改变其他人。HO 要求与人物采用一起确认；未分配点数须分配或明确确认处理。
3. **开始游戏**：检查缺项后，一次采用队伍并开始。服务端续办角色确认、建房、本地真人及 AI 席位、发布分配、独立 KP 档案和准备包绑定，并发布一次批准的公开开场。刷新或失败后继续原草稿，重复点击不会再建房或重放开场。

随机建卡由现有规则代码计算，模型只写人物文字；每人每次抽取最多 4 次实际文字调用，生成串行且显示实际进度。seed 复现建卡数值与随机选项，模型原文直接保存，游戏检定仍用独立的服务端骰子。失败保留数值卡和已完成成员，重试只补未完成部分；进程异常退出后，未完成的生成租约最多需 30 秒到期后才能续办。队伍采用后不提供直接重抽替换。

游戏桌面以剧情时间线、底部输入和待处理检定为主。角色状态、背包、已知线索及本人 HO 放在侧栏；单个本地真人自动成为操作身份。主机默认使用后端过滤的调查员视图，明确进入管理视图才读取其他 HO、KP 信息及调试。暂停、存读档、退出集中在游戏菜单，首页“继续”返回原房间状态。

多人在同一向导勾选邀请分支。朋友通过邀请码加入，选择已发布卡或上传自己的卡，等待主机接受后由本人准备；主机不能代远程成员准备。普通房间、角色工坊、Agent 档案、模组准备、知识库和系统状态的旧路由继续有效。

真人输入自然对话或行动后等待 KP；需要检定时按卡片掷骰，骰点和结果由服务端产生。模型失败时可在游戏菜单重试或取消回合；重试复用已完成结果和原调用预算。回合结束或等待检定时可存档，暂停后读档。已通过的流式重连、单气泡、输入幂等及记忆／结算机制继续沿用。第48批实现、实际验证与限制见[报告](docs/batch-48-report.md)。
...
4. 返回首页“开始新游戏”，选择刚导入的批准版本，配队伍并一次开始；不必逐页建席位、发布卡或创建 KP。游戏使用当前生效的模型配置。原房间管理中的手工绑定流程仍保留。
...
当前默认规则目录为 **1.5.0**：八项属性、**115 个职业**、**106 个目录技能 ID**、**6 类自定义专业**及**5种经历包**，分别计数。职业在原31项上补入84项手册条目；总类、别名及同一描述变体不重复计数。逐项公式、信用、分组、年代、来源及特殊条款剩余范围见 [职业覆盖表](docs/occupation-coverage.md)。随机模式使用3d6或2d6+6后乘5；购点使用规则书3.7方案四的**460点可选规则**，八项基础属性总和必须为460，范围15–90，INT/SIZ至少40，购点EDU至少15。幸运独立掷骰，不占属性池。规则配置使用 Pydantic 校验和安全 YAML 读取；公式仅接受封闭运算，没有 Python `eval`／`exec`。
...
职业点按条目采用 EDU×4 或 EDU×2＋指定／所选属性×2，兴趣点为 INT×2，分别记账；选经历包时增加独立经历池，技能值由基础及三池合法投入统一计算。信用评级须满足职业范围，母语基础值为EDU，闪避为DEX半值，职业／兴趣／经历点禁止购买克苏鲁神话；神秘学家可经KP许可独立获得初始值。未分配技能点在确认时放弃。当前不启用75技能上限、属性重骰等其他可选方案；没有逐项重骰入口。
...
刷新页面或重启后可继续读取草稿和确认后的角色，保留原始骰。旧 1.0.0–1.4.0 规则均已归档；旧草稿继续按原版编辑，旧卡导入按原版校验，旧确认卡和房间快照不迁移重算。

继续支持1920年代／现代信用评级现金、资产和日常消费额度、资产明细、六类背景及关键联系。统一目录有 **33项装备＝19项普通物品＋14项可自动结算武器**（徒手另计）：剑、斧、矛及4种普通步枪使用对应技能，车卡与主机准备共用服务端模板。来源、射程及具体未支持特例见[装备覆盖表](docs/equipment-coverage.md)。目录数量不代表完整武器规则完成。

车卡可按**每件枪械**配置初始已装弹／备弹，省略仍为0；服务端校验整数、非负、上限、枪械类别和容量，导入不能覆盖模板参数。开团首次结算实际实例；击发、装填、交接、丢弃拾回及读档保留同一实例的剩余状态。《常暗之厢》等模组起始遗失／选择规则仍优先。普通物品用途不自动产生技能或治疗效果；主机战斗面板展示基础射程并可选择原距离档位。第29批实现与实际验证范围见[报告](docs/batch-29-report.md)。

在“职业与技能 → 自定义专业”中选择类别并输入名称。服务端提供基础值：外语1、艺术／手艺5、科学1、驾驶1、生存10、学问1，上限99；目录内专业（包括数学基础10）直接选择，不能用同类别同名项改变基础值。添加后可选作原文允许的职业分组或兴趣专业；教授、图书馆管理员、医生的受限学术组允许不同科学专业各占一个名额，罪犯／士兵等混合方向组保留原方向限制。同一技能不能在固定项与多个组重复占位。改名保留ID和点数；删除清理选择与投入并退回点数。每卡最多50项，名称1–40字。

新版本自定义驾驶须由KP核对载具年代与背景；1920年代不接受已知现代飞行器名称。目录及自定义学问须经KP决定引入具体特殊知识领域，不能替代神话或授予法术。在本机车卡保存后，由主机勾选待引入项并再次保存；角色文件导入时清除外部核准，房间主机接受时重新核准列出的专业。改名或更改年代使旧核准失效，保留原投入。未经引入的学问不会以基础值自动进入检定候选。第28批定向测试与实际浏览器的确定性检定结果见 [报告](docs/batch-28-report.md)。

1.3.0新增两项职业特例：现代除魅师经KP许可，用催眠替换一个固定或已选职业技能，名额和点池不变；修改／撤销方案后，失去资格的职业点须修正或退回，不会转为兴趣点。神秘学家可申请由KP确定的初始神话，占个人特长名额、独立获得；建议不超过10%而非强制上限。职业原文未要求等额扣SAN，从POW重新推导并受99−神话上限约束（POW90、神话15为SAN84／上限84），不混入经历包的相信者扣减。

两种方案先保存查看服务端结果，再由主机明确许可；职业、内容、年代、分组、相关规则或影响初始SAN的属性变化会使对应许可失效。导入清除外部许可；仅待许可问题允许玩家预览／提交，主机逐项核准后在原事务中重算、确认、冻结并分配。初始神话保存在冻结卡，游戏期增长仍用原SAN增量，重复操作不叠加；KP上下文与实际检定显示同一当前神话值。详见[第30批报告](docs/batch-30-report.md)。

1.4.0启用四种非神话经历包，默认不选、最多一包且须KP同意：战场70点／SAN减1d10+5；警务、罪犯、医务各60点／SAN减1d10，最低年龄分别25、20、30。独立点池、来源白名单、背景与战场年份核对、一次持久原骰、初始SAN、明确本地许可、导入清除外部许可、提交核准及冻结贯通。符合批准遭遇独立类别的限定免疫进入实际SAN结算，免掷骰且不增加当日损失／神话；旧效果不自动分类。1.3.0原样归档，旧卡与冻结房间沿原版。来源、计算顺序及边界见[经历包说明](docs/experience-packages.md)，本批验收见[第32批报告](docs/batch-32-report.md)。

1.5.0完成**神话包创建／相信者状态／已知法术记录**：与四包互斥，KP手定初始神话或明确选择建议骰1D10+5，两项背景及初始法术由KP核准。初始来源分开重算；相信者从POW等额扣SAN后限上限，不信者仍受上限约束。批准神话遭遇的游戏期转换和玩家自愿相信进入正常SAN服务、一次性回执及存档；模型不能直接改状态。已知法术显示“尚不支持施放”。神秘学家初始来源与神话包叠加暂不支持，非官方禁止。来源计算表见[神话经历包第一阶段](docs/mythos-experience.md)，验收及失败复验见[第33批报告](docs/batch-33-report.md)。

普通手册职业数值块已有映射；**通用施法、法术学习／阅读、动物特殊裁定、其他专业替换**继续分别保留。尚未支持90岁及以上条款、自定义格斗／射击、跨专业可选加值，以及自动射击、霰弹分档、爆炸范围、链锯／绞索等武器特例；P2仍在开发清单单列。目录与来源见 [SOURCES.md](backend/app/rules/definitions/SOURCES.md)。神话包第一阶段完成不代表完整魔法系统完成。
...
4. 玩家在大厅“提交角色文件”选择从现有车卡工具导出的 **CharacterExport JSON**。先查看服务端重算的属性、派生值、点数及六类自定义专业，再点击“提交角色”。请求上限为 **256 KiB**；仅有专业待KP引入时可以提交，其他格式、版本或校验错误须在原车卡工具修正并重新导出。远程车卡编辑器尚未开发。
5. 主机在“玩家角色提交”打开该成员的当前预览，查看重算结果、来源、原骰及待引入专业，然后“接受并分配”或“拒绝提交”。接受同时核准界面列出的待引入专业，调用既有确认服务、冻结房间快照并初始化角色资源，自动分配给提交者。提交者已有角色时，先在“房间调查员”取消原分配。拒绝不创建角色库草稿；玩家可以提交新版本，旧版不能再被批准。
