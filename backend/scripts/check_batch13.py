...
All writes are isolated under .cache/batch-13/live. The existing local model is
...
DIRECTORY = ROOT / ".cache/batch-13" / os.environ.get("BATCH13_SESSION", "live")
if not DIRECTORY.resolve().is_relative_to((ROOT / ".cache/batch-13").resolve()):
    raise ValueError("Batch 13 isolation required")
BASE = "http://127.0.0.1:8013/api"
HOST = "batch13-isolated-acceptance-host"
...
def serve():
    ...
    uvicorn.run(create_app(settings), host="127.0.0.1", port=8013, log_level="warning")


def setup():
    if (DIRECTORY / "session.json").exists():
        raise ValueError("Keep the existing room; no reset")
    created = request("POST", "/rooms", {"name": "第十三批 · 停摆的钟楼自由互动"})
    prefix = "/rooms/" + created["room"]["id"]
    player_name = os.environ.get("BATCH13_PLAYER", "周岚")
    teammate_name = os.environ.get("BATCH13_TEAMMATE", "沈砚")
    joined = request(
        "POST",
        "/rooms/join",
        {
    ...
    room = request(
        "POST", prefix + "/members", {"display_name": teammate_name, "controller_type": "agent"}
    )["room"]
    teammate = next(m["id"] for m in room["members"] if m["controller_type"] == "agent")
    for name, member in [(player_name, player_id), (teammate_name, teammate)]:
    ...
    preparation = os.environ.get("BATCH13_PREPARATION")
    if preparation:
        request("PATCH", prefix + "/module-preparation", {"preparation_id": preparation})
    else:
    ...
    for member, profile in [
        ...
        (
            teammate,
            {
                "role": "investigator",
                "name": teammate_name,
                "personality": "谨慎务实，喜欢与同伴商量，偶尔用一句冷幽默缓和紧张。",
                "goals": "弄清眼前的异常原因",
    ...
    write(
        "scenario.json",
        {
            "module": preparation or "停摆的钟楼（仓库原创调查练习）",
            "start": "已准备局部场景" if preparation else "钟楼广场，雨夜",
            "characters": [player_name + "：真人调查员", teammate_name + "：AI调查员"],
            "predeclared": True,
    ...
    if args.command == "turn":
        ...
        with sqlite3.connect((DIRECTORY / "game.db").as_uri() + "?mode=ro", uri=True) as db:
            ...
            plans = [
                dict(row)
                for row in db.execute(
                    "select * from agent_action_plans where room_id=?",
                    (room["id"],),
                )
            ]
            for plan in plans:
            ...
            cycles = [
                dict(row)
                for row in db.execute(
                    "select * from agent_cycles where room_id=?",
                    (room["id"],),
                )
            ]
            for cycle in cycles:
        ...
        latest = json.loads(
            (DIRECTORY / "inputs.jsonl").read_text(encoding="utf-8").splitlines()[-1]
        )
        after = latest["event"]["seq"]
        for e in request("GET", prefix + f"/events?after_seq={after}&limit=1000", player=True)[
            "events"
        ]:
            if e["type"] in {
                "keeper.narration",
