...
BASE = ROOT / "data/prepared/changan/batch-35"
URL = "http://127.0.0.1:8035/api"
...
def serve(directory):
    ...
    server = uvicorn.Server(uvicorn.Config(create_app(settings), host="127.0.0.1", port=8035))
...
class Session:
    ...
    def setup(self):
        if not self.state.get("room_id"):
            created = self.req("POST", "/rooms", {"name": "第35批 常暗之厢 本地真实局"})
