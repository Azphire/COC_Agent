"""Batch 38 isolated real-model probes and normal API play, reusing batch 36 tools."""
...
BASE = ROOT / "data/prepared/changan/batch-38"
...
def version(directory):
    ...
    for folder in ("scripts", "tests"):
        for file in (ROOT / "backend" / folder).glob("*batch38*.py"):
...
def audit(directory):
    ...
    path.write_text(path.read_text(encoding="utf-8").replace("第36批", "第38批"), encoding="utf-8")
...
def main():
    ...
    if args.command == "serve":
        play_batch36.serve(directory, port=8038)
        return
    ...
    try:
        if args.command == "init":
            ...
            saved = session.req("POST", session.prefix + "/snapshots", {"name": "第38批途中保存"})
            ...
            name = args.args[0] if args.args else "第38批阻断现场"
