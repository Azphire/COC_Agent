                    ...
                    [
                        {
                            ...
                            "content": (
                                ...
                                "每个实体只引用一到两个直接支持它的 evidence_id，关系只引用一个；"
                                "不要将所有证据重复列入每个实体和关系，以免输出截断。"
                                "必须使用本批给出的 evidence_id，不编造证据、页码或事实。"
                                ...
                                "复制其id到existing_entity_id，并原样保留该目录项的title和type；"
                                "目录中没有的ID不得填写，不确定是否同一实体则留空，不能编造ID。"
